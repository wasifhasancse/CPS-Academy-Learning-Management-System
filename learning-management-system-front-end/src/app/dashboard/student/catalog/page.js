    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-base font-bold text-foreground">
            Explore Course Tracks ({catalogCourses.length})
          </h2>
          <p className="text-xs text-muted">
            Specialized tracks curated for ICPC competitors and software engineers
          </p>
        </div>
        <Link href="/courses">
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          description="There are currently no additional courses available in the catalog."
        />
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {catalogCourses.map((course) => {
            const isEnrolled = enrolledCourseIds.has(course.documentId || String(course.id));
            const lessonsCount =
              ...
              <Card key={course.documentId || course.id} className="flex flex-col justify-between">
                ...
                <CardContent className="pt-0 space-y-4">
                  ...
                  <Link href={`/courses/${course.slug || course.id}`} className="block">
                    <Button
                      variant={isEnrolled ? "surface" : "primary"}
                      size="sm"
                      className="w-full"
                    >
                      {isEnrolled ? "Already Enrolled • View Syllabus" : "View Course Details →"}
                    </Button>
                  </Link>
                </CardContent>
              </Card>
